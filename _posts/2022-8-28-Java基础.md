...
增加一个方法之后，上面就不是函数式接口了，可以看到 lambda 表达式就会报错。只有单个方法是函数式接口。虽然不能在函数式接口中定义多个方法，但可以定义默认方法、静态方法以及 java.lang.Object 里的 public 方法。
...
实例变量是储存在堆上面的，是线程共享的。而局部变量则是保存在栈上的，是线程不共享的。
