...
submit可以返回Future\<?>，调用这个future的get方法可以抛出异常。
...
[捕获异常](https://blog.csdn.net/qq_45076180/article/details/114552567)

建立自定义的ThreadFactory，设置setDefaultUncaughtExceptionHandler对象处理异常。
新建自定义线程池，使用自定义的线程工程。这个时候使用execute方法会打印异常。

也可以重写afterExecute处理execute提交的异常。